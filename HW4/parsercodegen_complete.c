...
void procedure_declaration(FILE *ofp);
...
void const_declaration(FILE * ofp){
    ...
    if(currentToken == 28){
        ...
        do{
            ...
            addSymbolTable(1, name, value, currentLevel, 0);
            nextToken();
...
int var_declaration(FILE * ofp){
    ...
    if(currentToken == 29){
        do{
            ...
            addSymbolTable(2, tokenList[currentIndex-1].lexeme, 0, currentLevel, numVars+2); 
            nextToken();
        }while(currentToken == 16);
...
void statement(FILE * ofp){
    ...
    // if token == callsym
        ...
        // if token != identsym
        if(currentToken != 2){
            printf("Error: const, var, procedure, and call keywords must be followed by an identifier\n");
            fprintf(ofp, "Error: const, var, procedure, and call keywords must be followed by an identifier\n");
            fclose(ofp);
            exit(0);
        }
        int symIdx = searchSymbolTable(tokenList[currentIndex-1].lexeme);
        ...
        if(symbol_table[symIdx].kind != 3){
            ...
            fprintf(ofp, "Error: call statement may only target procedures\n");
            fclose(ofp);
            exit(0);
        }

        // emit CAL 
        emit(5, currentLevel-symbol_table[symIdx].level, 3*symbol_table[symIdx].addr);
    ...
    // if token == ifsym
        ...
        // JMP
        emit(7, 0, 0);
        instr[jpcIdx].M = 3 * instrCodeIndex;

        if(currentToken != 33){
            printf("Error: if statement must include else clause\n");
            fprintf(ofp, "Error: if statement must include else clause\n");
            fclose(ofp);
            exit(0);
        }
        nextToken();
        ...
        if (currentToken != 23) { // fisym
            ...
            fprintf(ofp, "Error: else must be followed by fi\n");
            fclose(ofp);
            exit(0);
        }
...
void mark(void){
    for(int i = symbolCount - 1; i >= 0; i--){
        if(symbol_table[i].level == currentLevel && symbol_table[i].mark == 0){
...
void procedure_declaration(FILE *ofp){
    // if token == procsym
    while(currentToken == 30){
        nextToken();
        // if token != identsym
        if(currentToken != 2){
            printf("Error: const, var, read, procedure, and call keywords must be followed by an identifier");
            fprintf(ofp, "Error:onst, var, read, procedure, and call keywords must be followed by an identifier");
            fclose(ofp);
            exit(0);
        }
        ...
        if(currentToken != 17){
            ...
            fprintf(ofp, "Error: procedure declaration must be followed by a semicolon\n    ");
            fclose(ofp);    
            exit(0);
        }
        nextToken();
        ...
        addSymbolTable(3, name, 0, currentLevel, instrCodeIndex);

        // call BLOCK at deeper level
        currentLevel++;
        block(ofp);
        currentLevel--;
...
int main(void){
    ...
    for(int i=0; i<symbolCount; i++){
        int displayAddr = symbol_table[i].kind == 3 ? 3 * symbol_table[i].addr : symbol_table[i].addr;
        printf("   %d |\t      %s |     %d |     %d |       %d |    %d\n", symbol_table[i].kind, symbol_table[i].name, symbol_table[i].val, symbol_table[i].level, displayAddr, symbol_table[i].mark);
    }
